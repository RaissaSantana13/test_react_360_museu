...
import { EventItem } from '../../components/forms/evento/event-item';
import { CalendarView, type CalendarEvent } from '../../type/type';
import { useDictionary } from './i18n-providers';
...
export function CalendarDndProvider({ children, onEventUpdate }: CalendarDndProviderProps) {
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    // const viewLabels = {
    //   month: dict.event.CalendarDndProvider.month,
    //   week: dict.event.CalendarDndProvider.week,
    //   day: dict.event.CalendarDndProvider.day,
    //   agenda: dict.event.CalendarDndProvider.agenda,
    // } as const;
    ...
    setActiveView(view);
    setCurrentTime(new Date(calendarEvent.start_date));
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    try {
      ...
      if (!active.data.current || !over.data.current) {
        throw new Error(dict.event.calendarDndProvider.activeData_overData);
      }
      ...
      const originalStart = new Date(calendarEvent.start_date);
      const originalEnd = new Date(calendarEvent.end_date);
