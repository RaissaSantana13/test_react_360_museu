...
import { getBorderRadiusClasses, getEventColorClasses } from '../../../lib/event.utils';
...
function EventWrapper({
  ...
  const displayEnd = currentTime
    ? new Date(
        new Date(currentTime).getTime() + (new Date(event.end_date).getTime() - new Date(event.start_date).getTime()),
      )
...
export function EventItem({
  ...
  const displayStart = useMemo(() => {
    return currentTime || new Date(event.start_date);
  ...
  const displayEnd = useMemo(() => {
    return currentTime
      ...
      : new Date(event.end_date);
  }, [currentTime, event.start_date, event.end_date]);
  ...
  return (
    <button
      ...
      data-past-event={isPast(new Date(event.end_date)) || undefined}
      onClick={onClick}
      onMouseDown={onMouseDown}
      onTouchStart={onTouchStart}
      {...dndListeners}
