...
import { EndHour, StartHour, WeekCellsHeight } from '../../../service/constants/constants';
...
export function WeekView({ currentDate, events, onEventSelect, onEventCreate }: WeekViewProps) {
  ...
  const days = useMemo(() => {
    const weekStart = startOfWeek(currentDate, { weekStartsOn: 0, locale: selectedLocale });
    const weekEnd = endOfWeek(currentDate, { weekStartsOn: 0, locale: selectedLocale });
  ...
  const processedDayEvents = useMemo(() => {
    const result = days.map((day) => {
      ...
      const sortedEvents = [...dayEvents].sort((a, b) => {
        ...
        const aDuration = new Date(a.end_date).getTime() - aStart;
        const bDuration = new Date(b.end_date).getTime() - bStart;
          ...
          if (col.length === 0) {
            ...
            const overlaps = col.some((c) =>
              areIntervalsOverlapping(
                ...
                {
                  start: new Date(c.event.start_date),
                  end: new Date(c.event.end_date),
                },
              ),
            );
            if (!overlaps) {
    ...
    <div data-slot="week-view" className="flex h-full flex-col">
      <div className="bg-background/80 border-border/70 sticky top-0 z-30 grid grid-cols-8 border-b backdrop-blur-md">
        ...
        {days.map((day) => (
          ...
          >
            {/* Versão Mobile: Apenas a primeira letra do dia (ex: D 29, S 30) */}
            <span className="sm:hidden" aria-hidden="true">
              {format(day, 'EEEE', { locale: selectedLocale })[0].toUpperCase()} {format(day, 'd')}
            </span>

            {/* Versão Desktop: Nome abreviado (ex: Dom 29, Seg 30) */}
            <span className="max-sm:hidden">
              {(() => {
                const label = format(day, 'EEE', { locale: selectedLocale });
                const capitalized = label.charAt(0).toUpperCase() + label.slice(1);
                return `${capitalized} ${format(day, 'dd')}`;
              })()}
            </span>
          </div>
        ))}
        ...
        {days.map((day, dayIndex) => (
          ...
          >
            ...
            {(processedDayEvents[dayIndex] ?? []).map((positionedEvent) => (
              <div
                key={positionedEvent.event.idEvent}
